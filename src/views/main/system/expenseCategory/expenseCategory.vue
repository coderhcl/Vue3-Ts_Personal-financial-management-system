<template>
  <div class="expenseCategory">
    ...
    <div class="content">
      ...
      <el-table border :data="expenseTableData" stripe>
        <el-table-column
          type="index"
          label="序号"
          align="center"
...
export default defineComponent({
  ...
  setup() {
    ...
    const getCategoryList = async () => {
      const pageResult = await getPageListData("/expense/category/list", {
      ...
      if (pageResult.code > 0) {
        ...
        expenseTableData.value = pageResult.data.list
      }
    }
    // 改变分页时发送请求
    ...
    const addCategory = async () => {
      addCategoryForm.value?.validate(async (valid) => {
        if (valid) {
          const result = await addExpenseCategoryData({
    ...
    const editCategory = async () => {
      aditCategoryForm.value?.validate(async (valid) => {
        if (valid) {
          ...
          const result = await updateExpenseCategory(updateId.value, {
    ...
    const handleDeleteCategory = (row: any) => {
      ElMessageBox.confirm(`确定要删除分类 ` + row.name, "请确认删除", {
        ...
        const deleteCategoryResult = await deleteExpenseCategory(row._id)
